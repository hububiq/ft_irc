...
void Channel::addToAdmins(ServerDao &serv, Client &cli, std::string &newAdmin)
{
	...
	std::string reply = reply_factory::getReply(ERR_NOSUCHNICK, cli.getNickname(), newAdmin, "");
	cli.write_msg(reply);
}
...
std::vector<char> &Channel::getChanFlags()
{
	return this->_chanFlags;
}
...
void Channel::handleTurnL(Client &client, std::string &flagStr, int i, Message &msg)
{
	...
	std::string reply =
	    ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " MODE " + this->getChannelName() + " +l " + msg.params[2] + "\r\n";
...
void Channel::handleTurnO(Client &client, ServerDao &serv, Message &msg)
{
	...
	std::string reply =
	    ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " MODE " + this->getChannelName() + " +o " + msg.params[2] + "\r\n";
	this->broadcast(client, reply);
	client.write_msg(reply);
	if (msg.params.size() > 2)
		msg.params.erase(msg.params.begin() + 2);
...
void Channel::handleTurnK(Client &client, std::string &flagStr, int i, Message &msg)
{
	...
	std::string reply =
	    ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " MODE " + this->getChannelName() + " +k " + msg.params[2] + "\r\n";
	this->broadcast(client, reply);
	client.write_msg(reply);
}
...
void Channel::handleTurnT(Client &client, std::string &flagStr, int i)
{
	...
	this->addToChanFlags(flagStr[i]);
	std::string reply =
	    ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " MODE " + this->getChannelName() + " +t\r\n";
	this->broadcast(client, reply);
	client.write_msg(reply);
}
...
void Channel::handleTurnI(Client &client, std::string &flagStr, int i)
{
	...
	this->addToChanFlags(flagStr[i]);
	std::string reply =
	    ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " MODE " + this->getChannelName() + " +i\r\n";
	this->broadcast(client, reply);
	client.write_msg(reply);
}
...
void Channel::setFlagOn(ServerDao &serv, Client &client, Message msg)
{
	...
	for (size_t i = 1; i < flagStr.size(); ++i)
	{
		...
		{
			std::string reply = reply_factory::getReply(ERR_UNKNOWNMODE, client.getNickname(), std::string(1, flagStr[i]), "");
			client.write_msg(reply);
			continue;
		}
		if (flagStr[i] == 'i')
		{
			if (!this->isInviteOnly())
				this->handleTurnI(client, flagStr, i);
		}
		else if (flagStr[i] == 't')
		{
			if (!this->isTopicForOperator())
				this->handleTurnT(client, flagStr, i);
		}
		else if (flagStr[i] == 'k')
		{
			if (!this->isChannelKey())
			{
				if (!hasEnoughParams(client, msg))
					continue;
				this->handleTurnK(client, flagStr, i, msg);
			}
			else
			{
				std::string reply = reply_factory::getReply(ERR_KEYSET, client.getNickname(), this->getChannelName(), "");
				client.write_msg(reply);
			}
			if (msg.params.size() > 2)
				msg.params.erase(msg.params.begin() + 2);
		...
		else if (flagStr[i] == 'o')
		{
			...
			{
				std::string reply =
				    reply_factory::getReply(ERR_USERNOTINCHANNEL, client.getNickname(), msg.params[2], this->getChannelName());
				client.write_msg(reply);
				if (msg.params.size() > 2)
					msg.params.erase(msg.params.begin() + 2);
				continue;
		...
		else if (flagStr[i] == 'l')
		{
			if (!hasEnoughParams(client, msg))
				continue;
			if (!msg.params[2].empty() && msg.params[2].find_first_not_of("0123"
			                                                              "4567"
			                                                              "8"
			                                                              "9") != std::string::npos)
			{
...
void Channel::handleTurnOffK(Client &client)
{
	...
	this->_key.clear();
	std::string reply =
	    ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " MODE " + this->getChannelName() + " -k\r\n";
	this->broadcast(client, reply);
	client.write_msg(reply);
}
...
void Channel::handleTurnOffL(Client &client)
{
	...
	this->_limit = 0;
	std::string reply =
	    ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " MODE " + this->getChannelName() + " -l\r\n";
	this->broadcast(client, reply);
	client.write_msg(reply);
}
...
void Channel::handleTurnOffO(Client &client, Message &msg)
{
	...
	std::string reply =
	    ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " MODE " + this->getChannelName() + " -o " + targetNick + "\r\n";
	this->broadcast(client, reply);
	client.write_msg(reply);
	if (msg.params.size() > 2)
		msg.params.erase(msg.params.begin() + 2);
...
void Channel::setFlagOff(Client &client, Message msg)
{
	...
	for (size_t i = 1; i < flagStr.size(); ++i)
	{
		...
		{
			if (this->isInviteOnly())
				this->handleTurnOffI(client);
		}
		else if (flagStr[i] == 't')
		{
			if (this->isTopicForOperator())
				this->handleTurnOffT(client);
		}
		else if (flagStr[i] == 'k')
		{
			if (this->isChannelKey())
				this->handleTurnOffK(client);
			if (msg.params.size() > 2)
				msg.params.erase(msg.params.begin() + 2);
		}
		else if (flagStr[i] == 'o')
