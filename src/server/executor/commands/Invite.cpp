...
void Invite::execute(Client &client, Message &msg)
{
	...
	{
		std::string reply = reply_factory::getReply(ERR_NEEDMOREPARAMS, nickname, command, "");
		client.write_msg(reply);
		return;
	}
	if (!m_server->checkIfClientExistsByNickname(msg.params[0]))
	{
		std::string reply = reply_factory::getReply(ERR_NOSUCHNICK, nickname, msg.params[1], "");
		client.write_msg(reply);
		return;
	}
	...
	if (!ch)
	{
		std::string reply = reply_factory::getReply(ERR_NOSUCHCHANNEL, nickname, msg.params[0], msg.params[1]);
		client.write_msg(reply);
		return;
	}
	if (ch && !m_server->isUserInChannel(client, ch->getChannelName()))
	{
		std::string reply = reply_factory::getReply(ERR_NOTONCHANNEL, nickname, msg.params[1], "");
		client.write_msg(reply);
		return;
	}
	if (ch && m_server->isInviteeInChannel(msg, ch->getChannelName()))
	{
		std::string reply = reply_factory::getReply(ERR_USERONCHANNEL, nickname, msg.params[0], msg.params[1]);
		client.write_msg(reply);
		return;
	}
	if (!m_validator->isValidChannelName(msg.params[1]))
	{
		std::string reply = reply_factory::getReply(ERR_NOSUCHCHANNEL, nickname, msg.params[0], "");
		client.write_msg(reply);
		return;
	}
	if (ch && ch->isInviteOnly())
	{
		...
		{
			std::string reply = reply_factory::getReply(ERR_CHANOPRIVSNEEDED, nickname, msg.params[0], "");
			client.write_msg(reply);
			return;
		}
	}
	ch->addToInvited(msg.params[0]);
	std::string reply = reply_factory::getReply(RPL_INVITING, nickname, msg.params[0], msg.params[1]);
	client.write_msg(reply);
