...
void Channel::addToChanFlags(char flag) {
  std::vector<char>::iterator it =
      std::find(_chanFlags.begin(), _chanFlags.end(), flag);
...
void Channel::handleTurnL(Client &client, std::string &flagStr, int i,
  ...
  std::string reply = ":" + client.getNickname() + "!" + client.getUsername() +
                      ...
                      this->getChannelName() + " +l " + msg.params[2] + "\r\n";
...
void Channel::handleTurnO(Client &client, ServerDao &serv, Message &msg) {
  ...
  std::string reply = ":" + client.getNickname() + "!" + client.getUsername() +
                      "@" + client.getHostname() + " MODE " +
                      this->getChannelName() + " +o " + msg.params[2] + "\r\n";
  this->broadcast(client, reply);
  client.write_msg(reply);
  if (msg.params.size() > 2) msg.params.erase(msg.params.begin() + 2);
}
...
void Channel::handleTurnK(Client &client, std::string &flagStr, int i,
  ...
  std::string reply = ":" + client.getNickname() + "!" + client.getUsername() +
                      "@" + client.getHostname() + " MODE " +
                      this->getChannelName() + " +k " + msg.params[2] + "\r\n";
  this->broadcast(client, reply);
  client.write_msg(reply);
}
...
void Channel::handleTurnOffO(Client &client, Message &msg) {
  ...
  std::string reply = ":" + client.getNickname() + "!" + client.getUsername() +
                      "@" + client.getHostname() + " MODE " +
                      this->getChannelName() + " -o " + targetNick + "\r\n";
  this->broadcast(client, reply);
  client.write_msg(reply);
  if (msg.params.size() > 2) msg.params.erase(msg.params.begin() + 2);
}
...
bool Channel::isInvited(std::string clientToFind) {
  for (size_t i = 0; i < this->getInvited().size(); i++) {
    if (this->getInvited()[i] == clientToFind) return true;
  }
  return false;
}

bool Channel::isThereTopic() {
  if (this->_topic.size() != 0) return true;
  return false;
}
