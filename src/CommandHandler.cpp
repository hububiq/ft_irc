...
void CommandHandler::handleTopic(Client& client, Message& msg, Server& server) {
  ...
  if (chan && isUserInChan &&
      msg.params.size() ==
          1) {  // there is only one parameter and user is in channel
  ...
  if (isUserInChan && msg.params.size() >= 2) {
    ...
    // no flag "topic only by operators" - everybody can assign topic if its not
    // empty
    if (tempChan && !msg.params[1].empty()) tempChan->setTopic(msg.params[1]);
...
void CommandHandler::handleInvite(Client& client, Message& msg,
                                  Server& server) {
  std::string nickname = client.getNickname();
  std::string command = msg.command;
  ...
  for (it = cl.begin(); it != cl.end(); it++) {
    if (it->second.getNickname() == msg.params[0]) {
      std::string reply = prefix + " " + command + " " +
                          it->second.getNickname() + " :" + msg.params[1] +
                          "\r\n";
...
void CommandHandler::handlePrivMsg(Client& client, Message& msg,
                                   Server& server) {
...
/*TODO: check with Halloy if I need to sent RL_NOTOPIC if user joins canal
 * without topic*/
...
void CommandHandler::handleJoin(Client& client, Message& msg, Server& server) {
  ...
  if (ch == NULL) {  // channel will be freshly made so it has no topic - we
                     // dont send anythin to client which is joining
    Channel channel =
        Channel();  // could be refactored with parametrised constructor
    ...
    // ch->broadcast(client, reply) TODO here: add reply about joining to entire
    // chanel
    if (ch->isThereTopic()) {  // client succesfully joined, passed key if any
                               // set, now he will have viewed the channel topic
      std::string reply =
    ...
    std::string reply = Replies::getReply(
        RPL_NOTOPIC, nickname, msg.params[0],
        "");  // in case of no topic, information about it is shown to client
    client.write_msg(reply);
...
void CommandHandler::handleKick(Client& client, Message& msg, Server& server) {
  ...
  if (Parser::clientCanKICK(channel_obj, client)) {
    if (Parser::clientToBeKICKed(channel_obj, client_to_kick)) {
      ...
      std::cout << nick << " kicked " << client_to_kick << " form "
...
void CommandHandler::handleCommand(Client& client, Message& msg,
  ...
  std::map<std::string, void (*)(Client&, Message&, Server&)>::iterator it =
      commands.find(msg.command);
  if (it != commands.end())
