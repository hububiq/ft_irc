...
void Quit::execute(Client &client, Message &msg, Server &server)
{
	...
	{
		std::vector <Client *> &clients = it_channel_b->second.getMembers(); // for each channel I go throught list of clients
		std::vector <Client *>::iterator it_client = std::find(clients.begin(), clients.end(), &client); // &client to erase()
		if (it_client != clients.end()) 
		{
                        std::string replay = ":" + client.getNickname() + "!" + client.getUsername()
