...
void Mode::execute(Client &client, Message &msg)
{
	...
	if (msg.params.size() >= 2)
	{
		...
		if (m_server->isClientAdmin(client, chName))
		{
			...
			{
				std::string reply = reply_factory::getReply(ERR_UNKNOWNMODE, nickname, std::string(1, msg.params[1][0]), "");
				client.write_msg(reply);
			}
		}
		else
		{
			std::string reply = reply_factory::getReply(ERR_CHANOPRIVSNEEDED, nickname, chName, "");
			client.write_msg(reply);
		}
	}
}
