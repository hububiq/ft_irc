...
Client *Kick::clientToBeKICKed(Channel *channel_obj, const std::string &client_to_kick)
{
    std::vector<Client *> &temp_members = channel_obj->getMembers();
    for (std::vector<Client *>::iterator it = temp_members.begin(); it != temp_members.end(); ++it)
    {
        if ((*it)->getNickname() == client_to_kick)
            return *it;
    }
    return NULL;
}
...
void Kick::execute(Client &client, Message &msg, Server &server)
{
	...
	if (Kick::clientCanKICK(channel_obj, client))
	{
		...
		if (kicked)
		{
			std::string replay = ":" + nick + "!" + client.getRealName() + "@localhost"
						+ " KICK " + channel_name + " " + client_to_kick + " :" + reason + "\r\n";
			kicked->write_msg(replay); // this line notifies the kicked clinet first (that is kicked)
			...
			members.erase(std::find(members.begin(), members.end(), kicked));
                        ...
                        std::vector<Client *>::iterator it_a = std::find(admins.begin(), admins.end(), kicked);
                            if (it_a != admins.end())
                                admins.erase(it_a);
		}
		else
		{
			std::string err = Replies::getReply(ERR_USERNOTINCHANNEL, nick, channel_name, "");
			client.write_msg(err);
		}
	}
	else
        {
		std::string err = Replies::getReply(ERR_CHANOPRIVSNEEDED, nick, channel_name, "");
                client.write_msg(err);
        }
}
Kick globalKickCmd;
