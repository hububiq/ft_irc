...
void Kick::execute(Client &client, Message &msg)
{
	...
	{
		std::cerr << reply_factory::getReply(ERR_NEEDMOREPARAMS, client.getNickname(), "KICK", "");
		return;
	}
	...
	if (Kick::clientCanKICK(channel_obj, client))
	{
		...
		{
			std::string err = reply_factory::getReply(ERR_USERNOTINCHANNEL, nick, channel_name, "");
			client.write_msg(err);
		}
	}
	else
	{
		std::string err = reply_factory::getReply(ERR_CHANOPRIVSNEEDED, nick, channel_name, "");
		client.write_msg(err);
	}
}
