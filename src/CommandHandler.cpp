...
void CommandHandler::handleTopic(Client &client, Message &msg, Server &server)
{
	...
	{ // there is only one parameter and user is in channel
	...
	if (isUserInChan && msg.params.size() >= 2)
	{
		...
		// no flag "topic only by operators" - everybody can assign topic if its
		// not empty
		...
		if (!tempChan->getTopic().empty())
		{
			std::string reply = ":" + client.getNickname() + "!" + client.getUsername() + "@" +
			                    client.getHostname() + " TOPIC " + tempChan->getChannelName() + " :" +
			                    tempChan->getTopic() + "\r\n";
...
void CommandHandler::handleJoin(Client &client, Message &msg, Server &server)
{
	...
	if (ch == NULL)
	{ // channel will be freshly made so it has no topic - we
	...
	else if (ch != NULL)
	{
		...
		// ch->broadcast(client, reply) TODO here: add reply about joining to
		// entire chanel
		if (ch->isThereTopic())
		{ // client succesfully joined, passed key if any
...
void CommandHandler::handleKick(Client &client, Message &msg, Server &server)
{
	...
	if (Parser::clientCanKICK(channel_obj, client))
	{
		if (Parser::clientToBeKICKed(channel_obj, client_to_kick))
		{
			...
			std::string replay = ":" + nick + "!" + client.getRealName() + "@" + "localhost" + " KICK " + channel_name + client_to_kick + reason;
...
void CommandHandler::handleUser(Client &client, Message &msg, Server &server)
{
	...
	client.setRegister();
	std::string reply = Replies::getReply(RPL_WELCOME, client.getNickname(),
